...
uint32_t wifi_enable_press_time = 5000;
...
void ConfigManager::setDefaults() {
  ...
  longPressKeys.clear();
  // Default shortcuts
  longPressKeys.push_back(
      {0, 4, '\0', 0x09, '\0', 0x09, false, false, false, 0, 0}); // ALT -> Tab
  longPressKeys.push_back(
      {4, 5, 'm', '.', '.', '.', false, false, false, 0, 0}); // M -> ..
}
...
void ConfigManager::loadConfig() {
  ...
  if (!lpks.isNull()) {
    ...
    for (JsonObject lpk : lpks) {
      ...
      key.baseShortOutput =
          (bsc && bsc[0]) ? bsc[0] : (lpk["baseShortInt"] | 0);

      const char *blc = lpk["baseLong"];
      key.baseLongOutput = (blc && blc[0]) ? blc[0] : (lpk["baseLongInt"] | 0);

      const char *l2sc = lpk["layer2Short"];
      key.layer2ShortOutput =
          (l2sc && l2sc[0]) ? l2sc[0] : (lpk["layer2ShortInt"] | 0);

      const char *l2lc = lpk["layer2Long"];
      key.layer2LongOutput =
          (l2lc && l2lc[0]) ? l2lc[0] : (lpk["layer2LongInt"] | 0);
